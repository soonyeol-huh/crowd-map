...
function markerColor(population:number, level:Level) {
  ...
  if (ratio >= .78) return '#d7191c';
  if (ratio >= .58) return '#f57c00';
  if (ratio >= .38) return '#fbc02d';
  if (ratio >= .2) return '#00a6ca';
  return '#2c7bb6';
}
...
function App() {
  ...
  useEffect(() => {
    ...
    map.on('zoom',sync); map.on('moveend',sync);
    mapRef.current=map;
  ...
  useEffect(() => {
    const map=mapRef.current; const overlay=gridOverlay.current;
    if(!map||!overlay||!ready) return;
    overlay.innerHTML='';
    const elements = cells.map(cell=>{
      const el=document.createElement('button');
      el.type='button'; el.className='grid-cell'; el.style.background=gridColor(cell.population);
      el.innerHTML=`<strong>${cell.population.toLocaleString()}</strong><span>명</span>`;
      el.title=`${cell.name}: ${cell.population.toLocaleString()}명`;
      el.onclick=()=>setSelected(cell);
      overlay.appendChild(el);
      return {cell,el};
    });
    const draw=()=>{
      elements.forEach(({cell,el})=>{
        const nw=map.project([cell.west,cell.north]);
        const se=map.project([cell.east,cell.south]);
        el.style.left=`${nw.x}px`; el.style.top=`${nw.y}px`;
        el.style.width=`${Math.max(1,se.x-nw.x)}px`; el.style.height=`${Math.max(1,se.y-nw.y)}px`;
      });
    };
    draw(); map.on('render',draw);
    return ()=>{map.off('render',draw);overlay.innerHTML='';};
  },[cells,ready]);
